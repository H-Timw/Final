...
export default function CreateChessBoard() {
  ...
  >
    ...
    >
      <Space>
        <Typography.Title level={3} style={{margin:0}}>Input Size: </Typography.Title>
        <InputNumber value={inputValue} min={0} max={30} onChange={(value) => SetInputValue(value)}></InputNumber>
      </Space>
      <Space>
        <Typography.Title level={3} >Select color Player1: </Typography.Title>
        <ColorPicker value={color1} format='hex' onChange={(value) => setColor1(value.toHexString())}/>
      </Space>
      <Space>
        <Typography.Title level={3} >Select color Player2: </Typography.Title>
        <ColorPicker value={color2} format='hex' onChange={(value) => setColor2(value.toHexString())}/>
      </Space>
    </Space>
    <Space onClick={() =>{
