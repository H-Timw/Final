...
export default function Weather(){
  ...
  async function changeWeatherUI() {
    ...
    //change temperature
    setTemperature(Math.round((weather.main.temp-273 + Number.EPSILON) * 100) / 100)
    //change status
  ...
  return(
    <div className={temperature > 20 ? `weather-app bg-hot` : `weather-app bg-cold`}>
      <div id="weather" className={temperature > 20 ? `bg-hot` : `bg-cold`}>
        <Input defaultValue={"Ha Noi"} id="search-box" onChange={(e)=>getCitySearch(e.target.value)}/>
        <div>
          <h1 id="place">
        ...
        <Button type='primary' style={{backgroundColor:temperature>20?"#B35E1D":"#4F4FB5", fontSize:20, padding:'5px 30px',marginTop:10, height:'auto'}} onClick={changeWeatherUI}>Search</Button>
      </div>
    </div>
  )
}